...
def sub(all: str, left: str, right: str):
    a = all.find(left)
    b = all.find(right, a + len(left))
    return all[a + len(left): b]
...
def send(u: str, head: map = None):
    try:
        ...
        if ret.headers['Content-Type'] != 'image/jpeg' and ret.text.find("<script>window.location.href=") != -1:
            location = sub(ret.text, '<script>window.location.href="', '";')
            # print(location)
            ret = send(location, head)
        return ret
...
def saveImgAndFolder(url: str):
    ...
    if pagetitle == '':
        pagetitle = "无标题帖子统一目录"
    filePath = "./imgs/%s" % pagetitle.replace('/', '-')
...
def mainHandle():
    ...
    while True:
        ...
        if random.length > 0:
            a1_click = pq(random[0]).attr("onclick")
...
def imgServer():
    ...
    while True:
        ...
        if count == 4:
            strs = "-"
        log("[%s] 下载图片队列数量: %s，跳过重复或已缓存的Url: %s" %
            (strs, len(imgCaches), frezz))
        count += 1
        if len(imgCaches) == 0:
            sleep(.5)
            continue
...
if __name__ == '__main__':
    # send('https://shp.qpic.cn/collector/3318472577/0c0bee99-fc17-44b2-9a6d-6d7ea127f7d5/0')
    init()
